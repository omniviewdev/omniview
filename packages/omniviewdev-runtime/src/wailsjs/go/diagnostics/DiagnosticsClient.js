// @ts-check
// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT

export function Debug(arg1, arg2) {
  return window['go']['diagnostics']['DiagnosticsClient']['Debug'](arg1, arg2);
}

export function Error(arg1, arg2) {
  return window['go']['diagnostics']['DiagnosticsClient']['Error'](arg1, arg2);
}

export function Info(arg1, arg2) {
  return window['go']['diagnostics']['DiagnosticsClient']['Info'](arg1, arg2);
}

export function Log(arg1, arg2, arg3) {
  return window['go']['diagnostics']['DiagnosticsClient']['Log'](arg1, arg2, arg3);
}

export function ReadLog(arg1) {
  return window['go']['diagnostics']['DiagnosticsClient']['ReadLog'](arg1);
}

export function StartTail(arg1) {
  return window['go']['diagnostics']['DiagnosticsClient']['StartTail'](arg1);
}

export function StopTail(arg1) {
  return window['go']['diagnostics']['DiagnosticsClient']['StopTail'](arg1);
}

export function Warn(arg1, arg2) {
  return window['go']['diagnostics']['DiagnosticsClient']['Warn'](arg1, arg2);
}
