...
// AuthorizationManager is an interface that resource managers must implement
...
type AuthorizationManager[ClientT any] interface {
	...
	Create(ctx types.PluginContext, auth types.AuthorizationContext) error
	...
	List(ctx types.PluginContext) ([]types.AuthorizationContext, error)
...
// NewAuthorizationManager creates a new service to manage the various auth contexts for a
// plugin with an authenticated backend.
func NewAuthorizationManager[ClientT any](
	factory factories.ResourceClientFactory[ClientT],
) AuthorizationManager[ClientT] {
	return &authorizationManager[ClientT]{
		factory: factory,
		auths:   make(map[string]types.AuthorizationContext),
		clients: make(map[string]*ClientT),
	}
}

type authorizationManager[ClientT any] struct {
	factory factories.ResourceClientFactory[ClientT]
	auths   map[string]types.AuthorizationContext
	clients map[string]*ClientT
	sync.RWMutex
}

func (r *authorizationManager[ClientT]) Create(
	ctx types.PluginContext,
	auth types.AuthorizationContext,
) error {
	r.Lock()
	defer r.Unlock()
...
func (r *authorizationManager[ClientT]) Remove(
	ctx types.PluginContext,
	id string,
) error {
	r.Lock()
...
func (r *authorizationManager[ClientT]) List(
	_ types.PluginContext,
) ([]types.AuthorizationContext, error) {
	r.RLock()
	defer r.RUnlock()

	auths := make([]types.AuthorizationContext, 0, len(r.auths))
...
func (r *authorizationManager[ClientT]) GetContextClient(
	_ types.PluginContext,
	id string,
) (*ClientT, error) {
	r.RLock()
...
func (r *authorizationManager[ClientT]) RefreshContextClient(
	ctx types.PluginContext,
	id string,
) error {
	r.RLock()
...
func (r *authorizationManager[ClientT]) GetCurrentContextClient(
	ctx types.PluginContext,
) (*ClientT, error) {
	r.RLock()
	defer r.RUnlock()
...
func (r *authorizationManager[ClientT]) RefreshCurrentContextClient(
	ctx types.PluginContext,
) error {
	r.RLock()
	defer r.RUnlock()
