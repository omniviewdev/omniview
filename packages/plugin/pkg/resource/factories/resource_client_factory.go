...
type ResourceClientFactory[ClientT, OptionT any] interface {
	...
	// namespace.
	CreateClient(ctx context.Context, options OptionT) (*ClientT, error)
	...
	// invalidated.
	RefreshClient(ctx context.Context, client *ClientT, options OptionT) error
}
