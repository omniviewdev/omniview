...
import { Typography, Box, Checkbox, Dropdown, MenuButton, Menu, MenuItem, Divider, IconButton } from "@mui/joy";
import MoreHorizRoundedIcon from '@mui/icons-material/MoreHorizRounded';
import { LuChevronsLeftRight, LuFileCog, LuInspect, LuScrollText, LuTerminalSquare } from "react-icons/lu";
import { formatTimeDifference } from "@/utils/time";

/**
 * Render a standard text row for the generic resource table.
 */
export const TextRow = <T,>({ row, column }: { row: Row<T>, column: string }) => <Typography level="body-xs" noWrap>{row.getValue(column)}</Typography>
export const NumberRow = <T,>({ row, column }: { row: Row<T>, column: string }) => <Typography level="body-xs" noWrap>{row.getValue(column) || 0}</Typography>

/**
 * Render a selectbox for the header of the generic resource table.
 */
export const SelectBoxHeader = <T,>({ table }: { table: Table<T> }) => (
  <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', maxWidth: 24 }}>
    <Checkbox
      size="sm"
 ...
 */
export const SelectBoxRow = <T,>({ row }: { row: Row<T> }) => (
  <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', maxWidth: 24 }}>
    <Checkbox
      size="sm"
