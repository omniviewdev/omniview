...
const ContainerStatusCard: React.FC<Props> = ({ status }) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {status.state?.running && status.state.running.startedAt && (
      ...
      <CardContent>
        <Stack direction="column" spacing={0}>
          ...
          {status.state?.running && !status.lastState?.terminated && (
            <Typography level="body-sm">Container is healthy</Typography>
          )}
